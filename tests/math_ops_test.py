...
class LshSimilarityTest(tf.test.TestCase):
    ...
    def test_simple(self):
        ...
        real_fn_out = math_ops.lsh_similarity(x, y, t_vectors=t_vectors, t_group=t_group, nb_hyperplanes=nb_hyperplanes, threshold=0.00000001)
        grad_fn_out = math_ops.lsh_similary_back_fn(x, y, t_vectors=t_vectors, t_group=t_group, nb_hyperplanes=nb_hyperplanes)
        ...
        with tf.GradientTape() as tape:
            tape.watch(x)
            tape.watch(y)
            r = math_ops.lsh_similarity(x, y, t_vectors=t_vectors, t_group=t_group, nb_hyperplanes=nb_hyperplanes)
    ...
    def test_attn(self):
        ...
        real_fn_out = math_ops.lsh_attention(q, k, v, t_vectors_q=t_vectors, t_group=t_group, nb_hyperplanes=nb_hyperplanes, threshold=0.000000001)
        ...
        def test_func(_q, _k, _v):
            return math_ops.lsh_attention(_q, _k, _v, t_vectors_q=t_vectors, t_group=t_group, nb_hyperplanes=nb_hyperplanes, threshold=0.000000001)
        ...
        with tf.GradientTape() as tape:
            ...
            tape.watch(k)
            tape.watch(v)
            r = math_ops.lsh_attention(q, k, v, t_vectors_q=t_vectors, t_group=t_group, nb_hyperplanes=nb_hyperplanes)
